...
function installHome(){const grid=document.querySelector('.nh-grid-theory');if(!grid)return false;for(const t of tools){if(grid.querySelector(`[data-advanced-theory="${t.key}"]`))continue;const a=document.createElement('a');a.href=t.href;a.className='nh-tile';a.dataset.advancedTheory=t.key;a.innerHTML=`<span class="nh-icon">${t.icon}</span><span class="nh-tile-copy"><b>${t.home}</b><small>${t.desc}</small></span>`;grid.appendChild(a)}const group=grid.closest('.nh-tool-group');const count=group?.querySelector('.nh-group-head small');if(count)count.textContent='12 tools';const status=document.querySelector('.nh-status span:first-child');if(status)status.textContent='20 TOOLS';return true}
