...
const triggers=[...document.querySelectorAll('#menuOpenBtn,#openMenu,.menu-button,[aria-label*="メニューを開く"]')];triggers.forEach(btn=>{btn.classList.add('neet-menu-hamburger');btn.innerHTML='☰';btn.setAttribute('aria-label','メニューを開く');btn.setAttribute('aria-expanded','false');btn.onclick=e=>{e.preventDefault();open();btn.setAttribute('aria-expanded','true')}});
...
nav.querySelector('#neetHelpMenu').onclick=()=>alert('ノート・歌詞・録音・アイデア・制作ツール・制作カレンダーを、同じメニューから移動できるよ。');
