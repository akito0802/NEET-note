...
// 既存メニューを非表示にして共通メニュー1個だけにする
const hideOldMenus=()=>{
  document.querySelectorAll('#sideMenu,#menuOverlay,#siteSideMenu,#siteMenuOverlay,.neet-side-menu,.neet-menu-overlay,.neet-menu-trigger,.neet-menu-open,#open.menu-button,#open.icon').forEach(el=>{
    if(!el.closest('.ngm-menu')&&!el.classList.contains('ngm-btn')) el.style.display='none';
  });
};
hideOldMenus();
