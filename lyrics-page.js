...
function saveNow(){clearTimeout(timer);if(!currentId)return;const index=notes.findIndex(note=>note.id===currentId);if(index<0)return;if((notes[index].title||'')===titleInput.value.trim()&&(notes[index].text||'')===text.value){status.textContent='保存済み';return}notes[index]={...notes[index],title:titleInput.value.trim(),text:text.value,updatedAt:new Date().toISOString()};persist();status.textContent='保存済み';render()}
