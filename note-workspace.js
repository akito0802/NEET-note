...
function stripRedundantBlocks(){
 document.querySelectorAll('#noteWorkspaceIntro,.neet-home,#neetHomeDashboard').forEach(el=>el.remove());
 document.querySelectorAll('section,div,article').forEach(el=>{
  const t=(el.textContent||'').replace(/\s+/g,' ').trim();
  if((t.includes('思いついた音を、すぐ形に。')&&t.includes('QUICK ACCESS'))||
     (t.includes('作曲・歌詞・録音・アイデアを、それぞれ使いやすい場所に保存しよう。')&&t.includes('新しい曲を作る'))){
    el.remove();
  }
 });
}
stripRedundantBlocks();
new MutationObserver(stripRedundantBlocks).observe(document.body,{childList:true,subtree:true});
